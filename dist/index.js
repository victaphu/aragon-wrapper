...
class Aragon {
  constructor(daoAddress, options = {}) {
    ...
    this.cache = new _cache.default(cachePrefix); // Set up app contexts
    ...
    (0, _operators.scan)(({
      ...
      const {
        appId: updatedAppId
            ...
            update = (0, _objectWithoutProperties2.default)(repoUpdate, _excluded);
  ...
  async getExternalTransactionPath(destination, methodAbiFragment, params) {
    ...
    try {
      const tx = await (0, _transactions.createDirectTransaction)(account, destination, methodAbiFragment, params, this.web3);
  ...
  async getACLTransactionPath(methodSignature, params) {
    ...
    const method = (0, _apps2.findAppMethodFromSignature)(acl, methodSignature, {
    ...
    if (method.roles && method.roles.length !== 0) {
      ...
      const methodAbiFragment = (0, _abi.findMethodAbiFragment)(acl.abi, methodSignature);
  ...
  async calculateTransactionPath(sender, destination, methodSignature, params, finalForwarder) {
    ...
    const method = (0, _apps2.findAppMethodFromSignature)(app, methodSignature, {
      allowDeprecated: false
    });
    ...
    const directTransaction = await (0, _transactions.createDirectTransactionForApp)(sender, app, methodSignature, params, this.web3);
    const finalForwarderProvided = (0, _web3Utils.isAddress)(finalForwarder); // We can already assume the user is able to directly invoke the action if:
