...
exports.FORWARD_SIG = void 0;
const FORWARD_SIG = '0xd948d468'; // function forward(bytes)
...
exports.FORWARD_SIG = FORWARD_SIG;
...
function isValidForwardCall(calldata) {
  ...
  const selector = calldata.substring(0, 8); // Drop selector and grab the argument data

  const evmscriptData = calldata.substring(8); // Since arrays of bytes are encoded following the [offset][length][data]
...
function parseForwardCall(calldata) {
  ...
  const evmscriptData = calldata.substring(8); // Parse first word of the bytes array to get data offset
  // (it's stored as bytes so we need to parse in hex first and then multiply by 2)

  const offset = parseInt(`0x${evmscriptData.substring(0, 64)}`, 16) * 2; // The first word in the data is its length (uint256); actual data starts after

  const startIndex = offset + 64; // Parse length of the data stored
  // (it's stored as bytes so we need to parse in hex first and then multiply by 2)

  const dataLength = parseInt(`0x${evmscriptData.substring(offset, startIndex)}`, 16) * 2; // Grab the data stored in the bytes array
