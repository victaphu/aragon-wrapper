...
exports.findMethodAbiFragment = findMethodAbiFragment;

function findMethodAbiFragment(abi, methodSignature) {
  ...
  const methodAbiFragment = abi.filter(method => method.type === 'function').find(method => {
    ...
    const currentParameterTypes = method.inputs.map(({
      type
    }) => type);
